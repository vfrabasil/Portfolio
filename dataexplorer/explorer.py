...
# HEROKU:
mylogo = './dataexplorer/logoHeat.png'
samplef1 = './dataexplorer/f210708.csv'
samplef2 = './dataexplorer/f210715.csv'
inputcsv = './dataexplorer/img1.png'
outputcsv = './dataexplorer/img2.png'

# LINK:
#mylogo = '.\images\logohd.png'
...
#def main():
    ...
    if stlit == True:
        ...
        if page == ' ':
            ...
            st.write("""
            App para analizar los codigos de respuesta de transacciones obtenidos en dos días distintos 
            y evaluar la variacion en los valores entre ambos.
            """)
            st.write("""
    ...
    if st.checkbox('Reducir matriz', value=False):
        if stlit == False:
            ...
            with st.form(key='my_form1'):
                optionals = st.expander("📏 Reducir por nivel de tolerancia:", False)
                #threshold = optionals.slider( "Umbral (threshold)", float(threshold), float(0), float(threshold * 0.1) )


                selMin, selMax = optionals.slider( "Umbral (threshold)", float(ratioMin), float(ratioMax), (selMin, selMax) )
                threshold = selMax


                with st.expander("📋 Por codigo de transaccion o terminal (Fila/Col)  :", expanded=False):
